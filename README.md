...
段落开头写满 48 个字符就会打分，即使模型从不换行。第一段已经在说 `we` / `we need` / `we will`，与 `Let me` 同类，直接判为旧模型。后出现的 `Let me` **会推翻** 先前的保留——先看起来像灰度、后来开始说 `Let me` 的探测会被中止并按开关删除，而不会继续以绿色「灰度」停在 0%。一轮 **结束** 时仍未命中灰度的一律丢弃——50% 的灰色地带只留给还在积累开头的进行中探测。
...
**左栏** —— 探测提示词、模型（默认 V4-Pro / High）、并发数、存放目录、评分阈值，以及开关：命中强匹配时自动暂停、思维链以中文为主时丢弃、自动丢弃（判为旧模型即中止；关掉以便检查误杀）、从磁盘删除旧模型会话。
