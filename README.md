...
- 左栏：**探测提示词**、**模型**（默认 V4-Pro / High）、**并发数**、**最大探测数**、**存放目录**，以及评分阈值与开关（`命中一次后停止`、`自动删除旧模型会话`）。
- 右栏：顶部统计（已发起 / 进行中 / 已保留 / 已丢弃 / 最高分），下方是探测队列——每条都带 **分数条**（标出两条阈值线）、命中的短语（正/负分色）、状态与思维链预览；点击任意一条可直接打开该会话。
- **开始 / 停止 / 清空已结束**。停止后，进行中的探测会各自跑到自己的判定为止，只是不再发起新的。
