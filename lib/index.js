...
const DEFAULT_CONFIG = Object.freeze({
  ...
  // after the 5s grace window unless the user pinned/saved the row.
  autoDelete: false,
});
...
function publicAttempt(attempt) {
  return {
    ...
    saved: !!attempt.saved,
    graceUntil: attempt.graceUntil ?? null,
  };
}
...
function pump(ctx) {
  ...
  while (state.running && activeCount() < state.config.concurrency) {
    ...
    if (state.attempts.length > HISTORY_LIMIT) {
      for (const dropped of state.attempts.slice(HISTORY_LIMIT)) {
        if (dropped.graceTimer) clearTimeout(dropped.graceTimer);
      }
...
function clearGrace(attempt) {
  if (attempt.graceTimer) {
    clearTimeout(attempt.graceTimer);
    attempt.graceTimer = null;
  }
  attempt.graceUntil = null;
}

/** Really cancel the probe and optionally delete the session log. */
...
/**
 * Classifier wants this gone: wait 5s with the turn still running so the
 * user can pin it and read the chain-of-thought. No Keep → then cancel.
 */
function offerGrace(ctx, attempt, reason) {
  if (attempt.pinned || attempt.saved) return;
  ...
  attempt.graceUntil = Date.now() + GRACE_MS;
 ...
 * into the 5s grace window rather than cancelling on the same tick.
 */
function retractKeep(ctx, attempt, reason) {
  if (attempt.pinned || attempt.saved) return;
  const pausedForHit = state.paused && state.note === 'hit';
  offerGrace(ctx, attempt, reason);
...
function findAttempt(id) {
  const n = Number(id);
  const attempt = state.attempts.find((a) => a.id === n);
...
function pinAttempt(id) {
  ...
  attempt.pinned = true;
  if (attempt.closed) {
    attempt.status = attempt.saved ? 'saved' : 'pinned';
...
function saveAttempt(id) {
  ...
  attempt.pinned = true;
  attempt.saved = true;
  if (attempt.closed) attempt.status = 'saved';
  else if (attempt.status === 'pending-discard' || attempt.status === 'discarding') attempt.status = 'streaming';
  return publicState();
}

function discardOne(ctx, id) {
  commitDiscard(ctx, findAttempt(id));
  return publicState();
}
...
function evaluate(ctx, attempt, final) {
  ...
  // A user-pinned row is theirs to discard or save; the classifier
  // keeps scoring it but does not cancel.
  if (attempt.pinned || attempt.saved) return;

  // A keep is not final: "I'll" / a high score can land first, then a
  // later "Let me" opening reveals the old model. Retract into the 5s
  // grace window instead of cancelling on the same tick.
  if (attempt.decided) {
...
function finish(ctx, attempt) {
  ...
  if (!attempt.decided) {
    ...
    if (attempt.forced || attempt.error) {
      ...
      if (openings >= state.config.minOpenings && attempt.score >= state.config.keepAbove) {
        ...
        offerGrace(ctx, attempt, attempt.positive === 0 ? 'window' : 'ended');
        settle(ctx, attempt);
        return;
      }
    }
  ...
  } else if (attempt.status === 'discarding' || attempt.verdict === 'old') {
    attempt.status = 'discarded';
    if (state.config.autoDelete && !attempt.pinned && !attempt.saved) {
      deleteAttempt(ctx, attempt).catch((error) => {
        attempt.error = error instanceof Error ? error.message : String(error);
      });
    }
...
async function clearHistory(ctx) {
  ...
  const drop = [];
  for (const attempt of state.attempts) {
    if (isLive(attempt) || attempt.pinned || attempt.saved) {
      keep.push(attempt);
    } else {
      drop.push(attempt);
    }
    ...
    if (request.method === 'POST') {
      ...
      switch (body.action) {
        ...
        case 'pin':
          ...
          return;
        case 'save':
          respondJson(response, 200, saveAttempt(body.id));
          return;
        case 'discard-one':
          respondJson(response, 200, discardOne(ctx, body.id));
          return;
        default:
          throw new TypeError('action 必须是 start / pause / resume / force-stop / clear / delete-all / pin / save / discard-one');
      }
    }
