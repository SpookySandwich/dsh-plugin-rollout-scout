...
//
// The two decisive signals are not symmetric.
//
// "Let me" opening ANY paragraph settles the probe as the old model. "I'll"
// only counts as proof when it opens the WHOLE chain-of-thought: old-model
// reasoning happily opens a middle paragraph with "I'll create a single HTML
// file…" and then says "Let me build…" and "Let me write…" further down, so
// treating any "I'll" as proof produced false positives. Elsewhere it is just
// a positive signal that has to win on the balance of openings.
...
const NEGATIVE_OPENING = /\b(?:Let me|Let's|Let us|We need|We should|We'll|We can)\b/i;
...
/**
 * Split a chain-of-thought into paragraphs. While streaming, the final
 * paragraph is dropped because its opening may still be half-written; once
 * the turn has ended nothing more is coming, so every paragraph counts —
 * without that, a chain-of-thought whose decisive opening happens to be its
 * last paragraph would never be judged on it at all.
 */
...
function classify(text, final) {
  ...
  for (let index = 0; index < paragraphs.length; index += 1) {
    ...
    // Only the very first paragraph can prove the rollout model.
    if (index === 0 && DECISIVE_NEW_FIRST.test(opening)) decisive = 'new';
    if (decisive === null && DECISIVE_OLD.test(opening)) decisive = 'old';
...
function finish(ctx, attempt) {
  ...
  if (!attempt.decided) {
    // Re-run over the complete text: the streaming pass always withholds the
    // final paragraph, and its opening can be the decisive one.
    evaluate(ctx, attempt, true);
  }
  if (!attempt.decided) {
    // Still nothing decisive. Judge on the final score, under the same
    // evidence floor the live path uses.
    const openings = attempt.positive + attempt.negative;
    attempt.verdict = openings < state.config.minOpenings ? 'unknown'
    ...
    if (request.method === 'POST') {
      ...
      switch (body.action) {
        ...
        case 'clear':
          ...
          return;
        default:
          throw new TypeError('action 必须是 start / pause / resume / force-stop / clear');
      }
    }
