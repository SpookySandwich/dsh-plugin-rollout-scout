  ...
  apply(ctx) {
    ...
    function AttemptCard(props) {
      ...
      React.useEffect(function () {
        return function () {
          if (hovered.current && props.onRelease) props.onRelease(a.id, hoverLease.current);
        };
      }, []);
    ...
    function ScoutView(props) {
      ...
      const refreshedRevision = React.useRef(null);
      const sessionsRevision = remote ? remote.sessionsRevision : null;
      React.useEffect(function () {
        if (sessionsRevision === null || refreshedRevision.current === sessionsRevision) return;
        refreshedRevision.current = sessionsRevision;
        try {
      ...
      // Any owned agent blocks disk deletion until host teardown completes.
